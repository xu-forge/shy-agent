...
- [ ] **A1** 是否仅个人单用户？要不要多 profile？
- [ ] **A2** 主要体验对标（便于对齐交互细节）？

### B. 模型 / 编排

- [x] **B1** OpenAI-compatible（Minimax 等）
- [ ] **B3** 是否需要多 Agent / 子 Agent（LangGraph 多节点之外的协作）？

### C. 记忆细节

- [x] **C2** 短期记忆 = 保关键上下文压缩（原则已定；算法细节进 change）
- [ ] **C3** 当前上下文是否包含工作区、终端、浏览器等环境快照？
- [ ] **C5** 记忆是否必须本地、禁止上传云端？
- [x] **C4** SQLite + 文件混合；向量库延后
- [x] Agent 维护长期记忆：**默认可写，必须告知用户**；用户可事后改/删

### D. 工作流细节

- [x] **D2** Windows + macOS
- [ ] **D5** 长工作流：仅运行时计划，还是可保存工作流定义？是否要暂停/恢复？
- [ ] 高危操作清单：除删除外，是否包含覆盖写、执行未知脚本、外发网络、安装软件等？

### E. Skills 来源

- [ ] **E2** 技能来源：仅本地目录，还是 Git / 市场？
...
- [ ] **G1** 聊天主界面 + 侧栏（**含长期记忆管理** / 技能 / 轨迹）是否足够？要不要独立任务视图？
- [ ] **G3** 托盘常驻 / 全局快捷键？
