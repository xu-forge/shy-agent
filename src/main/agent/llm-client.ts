...
export function zodShapeToJsonSchema(shape: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(shape)) {
    ...
    if (t === 'string') {
      ...
      const entries =
        (field as { entries?: Record<string, string> }).entries ??
        (field._def as { entries?: Record<string, string> } | undefined)?.entries ??
        {}
      ...
      const innerShape =
        (field as { shape?: Record<string, unknown> }).shape ??
        (field._def as { shape?: Record<string, unknown> } | undefined)?.shape ??
        {}
