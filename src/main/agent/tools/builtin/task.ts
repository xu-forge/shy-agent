...
import { SUBAGENT_MAX_CONCURRENT, type SubagentType } from '../../subagent/types'
  ...
  registerTool(
    ...
    (ctx) =>
      new DynamicStructuredTool({
        ...
        func: async (input: { task_id: string; blocking?: boolean }) => {
      ...
      new DynamicStructuredTool({
        ...
        schema: z.object({
          ...
          parent_session_id: z
            .string()
            .optional()
            .describe('列查：父 session id（默认当前 session）')
        }),
        func: async (input: { task_id?: string; parent_session_id?: string }) => {
          if (input.task_id) {
  ...
  registerTool(
    ...
    (_ctx) =>
      new DynamicStructuredTool({
        ...
        func: async (input: { task_id: string }) => {
