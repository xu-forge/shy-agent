    ...
    (ctx) =>
      new DynamicStructuredTool({
        ...
        description:
          ...
          '参数：`command` 必填（要执行的命令字符串）；`cwd` 可选（默认当前工作目录）。',
        schema: z.object({
          command: z.string(),
    ...
    (ctx) =>
      new DynamicStructuredTool({
        ...
        description:
          ...
          '参数：`path` 必填（绝对路径或相对 cwd）；`maxChars` 可选（默认 50000）。',
    ...
    (ctx) =>
      new DynamicStructuredTool({
        ...
        description:
          ...
          '注意：是「覆盖」不是「合并」；误用会丢内容。\n' +
          '参数：`path` 必填（写入位置）；`content` 必填（完整新内容）。',
    ...
    (ctx) =>
      new DynamicStructuredTool({
        ...
        description:
          ...
          '不可恢复（不进回收站）；删错了只能从 git/restore 找回。\n' +
          '参数：`path` 必填（绝对路径或相对 cwd）；`recursive` 可选（默认 false，仅删文件；true 时删目录及子项）。',
