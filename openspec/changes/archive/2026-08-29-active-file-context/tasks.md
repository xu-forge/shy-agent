...
- [x] 2.1 `CodeWorkspace` 向父级回调当前 `activePath`（无 tab 时 `null`）；关闭最后 tab 清掉
- [x] 2.2 `MaterialLibrary` 向父级回调 lightbox 打开时的 `relativePath`，关闭时 `null`；lightbox 内切换文档同步更新
- [x] 2.3 `App` 按当前工作区 kind 汇总 `activeView`；`ChatWorkspace.onSend` 写入 `chat({ ..., activeView })`；无查看文件则省略字段

## 3. 验收

- [x] 3.1 `npm run typecheck` 通过
- [x] 3.2 `npm test` 通过
