...
- To: 图一色板（主色 `#4ADE80`、背景 `#F8FAFC`）；64px 图标轨 + 二级侧栏按项目分组（含「未选择项目」）
- Reason: 对齐设计稿，并为代码文件树腾出二级栏
...
**代码 / 素材工作区**
- From: 无
- To: 代码 = 文件树 + Monaco（可编辑保存）+ 右侧会话；素材 = `MaterialItem` 网格 + 查看器壳 + 右侧会话；绑定后 Agent `workspaceDir` = `rootPath`
