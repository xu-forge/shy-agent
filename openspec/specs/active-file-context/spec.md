...
#### Scenario: 无关则忽略
- **WHEN** 本轮带有 `activeView` 但用户问题与该文件无关
- **THEN** 规则文本 MUST 要求忽略该块且不要主动提及该文件
